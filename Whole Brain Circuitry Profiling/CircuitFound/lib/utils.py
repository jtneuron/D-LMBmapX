...
def get_conf():
    ...
    for key in conf:
        if conf[key] is None:
            ...
            if key == 'gpu':
                parser.add_argument('--gpu', type=str, default=conf[key])
                ...
                parser.add_argument('--distributed', type=bool, default=conf[key])
            else:
    ...
    if args.gpu:
        ...
        # if args.distributed:
        #     args.gpu = [int(gpu_id.strip()) for gpu_id in args.gpu.split(',')]
        # else:
        #     args.gpu = int(args.gpu)
