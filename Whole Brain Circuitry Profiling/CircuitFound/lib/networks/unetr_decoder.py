...
class UNETR_decoder(nn.Module):
    ...
    def forward(self, x_in, x, hidden_states_out):
        ...
        # x:(1,512,768) == hidden_states_out[12]
        # enc1:(1,16,128,128,128)
        enc1 = self.encoder1(x_in)
        # x2:(1,512,768)
        x2 = hidden_states_out[3]

        # self.proj_feat(x2, self.hidden_size, self.grid_size): (1, 768, 8, 8, 8)
        # enc2:(1, 32, 64, 64, 64)
        ...
        # # 使用三线性插值进行上采样
        # enc1_upsampled = F.interpolate(enc1, size=(160, 160, 160), mode='trilinear', align_corners=False)
        # # dec0: (1, 16, 160, 160, 160)
        # dec0 = self.decoder2(dec1, enc1_upsampled)
        # # 使用三线性插值进行下采样
        # dec0_resized = F.interpolate(dec0, size=(150, 150, 150), mode='trilinear', align_corners=False)
        # # out: (1, 2, 150, 150, 150)
        # output = self.out(dec0_resized)
