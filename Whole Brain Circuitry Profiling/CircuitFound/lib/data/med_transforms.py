...
def get_scratch_train_transforms(args):
    if args.dataset in ['btcv', 'TH_mixed', 'TH_P28']:
        train_transform = transforms.Compose(
            [
                transforms.LoadImaged(keys=["image", "label"]),
                transforms.AddChanneld(keys=["image", "label"], allow_missing_keys=True),
        ...
        train_transform = transforms.Compose(
            [
                ...
                transforms.AddChanneld(keys=["label"], allow_missing_keys=True),
                transforms.Orientationd(keys=["image", "label"], axcodes="RAS"),
                transforms.Spacingd(
                    keys=["image", "label"],
                    pixdim=(1.0, 1.0, 1.0),
...
def get_val_transforms(args):
    val_transform = transforms.Compose(
        [
            ...
            transforms.ToTensord(keys=["image", "label"], allow_missing_keys=True),
        ]
    )
...
def get_vis_transforms(args):
    val_transform = transforms.Compose(
        [
            ...
            #     allow_missing_keys=True
            # ),
            transforms.ToTensord(keys=["image"], allow_missing_keys=True),
        ]
    )
    return val_transform
