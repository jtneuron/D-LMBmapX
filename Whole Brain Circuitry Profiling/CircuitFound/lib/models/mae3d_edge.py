...
class MAE3DEdge(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoder_edge = decoder(patch_size=patch_size,
                                     num_classes=out_chans,
                                     embed_dim=args.decoder_embed_dim,
                                     depth=args.decoder_depth,
                                     num_heads=args.decoder_num_heads)
        ...
        self.edge_loss = DiceCELoss(sigmoid=True,
    ...
    def forward(self, images, edge_label, return_image=True):
        args = self.args
        ...
        l_x = patchify_image(edge_label, self.patch_size)
        ...
        # 1. 只计算mask部分的重建的loss，输入的edge label分块，不行，输入encoder的只有原图。。。
        # 2. edge作为输入的一部分，multi-modal + multi-task
        # 3. 串行，分两部进行预训练，第一步原图重建，第二部在edge上做微调
        # 4. 直接multi-task，参考hog论文！！！两个decoder，最后linear层不一样，但是这里好像是一样的
        # 5. 只做edge提取，单个decoder
        # 操作1，不行换5
        # all_x_edge shape: torch.Size([32, 512, 4096])
        all_x_edge = self.decoder_edge(all_x)
        patch_size = to_3tuple(self.patch_size)

        msk_all_x_edge_p = all_x_edge[:, -msk_length:, :].contiguous().view(batch_size * msk_length, 1, *patch_size)  # [32 * 512, 16, 16, 16]

        msk_label_x_p = msk_label_x.contiguous().view(batch_size * msk_length, 1, *patch_size)  # [32 * 128, 16, 16, 16]
        # msk_label_x_p = msk_label_x.view(batch_size, msk_length, 1, *patch_size).view(batch_size * msk_length, 1, *patch_size)  # [32, 512, 16, 16, 16]

        self.loss_edge = self.edge_loss(input=msk_all_x_edge_p, target=msk_label_x_p)
        self.loss = (1 - args.edge_weight) * self.loss_pxiel + args.edge_weight * self.loss_edge
        ...
        if return_image:
            ...
            # here
            recon_pixel = torch.cat([self.patch_norm(shuffled_x[:, :sel_length, :]), all_x_pixel[:, -msk_length:, :]], dim=1) \
                .gather(dim=1, index=unshuffle_indices[:, :, None].expand(-1, -1, out_chans))


            recon_edge = torch.cat([self.patch_norm(shuffled_label_x[:, :sel_length, :]), all_x_edge[:, -msk_length:, :]], dim=1) \
                .gather(dim=1, index=unshuffle_indices[:, :, None].expand(-1, -1, out_chans))
            # recon_edge = all_x_edge.gather(dim=1, index=unshuffle_indices[:, :, None].expand(-1, -1, out_chans))


            masked_x = unpatchify_image(masked_x, self.patch_size, original_shape)
            # here
            recon_pixel = unpatchify_image(recon_pixel, self.patch_size, original_shape)
            recon_edge = unpatchify_image(recon_edge, self.patch_size, original_shape)

            # here
            return self.loss, masked_x, recon_pixel, recon_edge
            # return self.loss_edge, masked_x, recon_edge
        else:
            # here
            return self.loss, self.loss_pxiel, self.loss_edge
