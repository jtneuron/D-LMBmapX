...
class MAE3D(nn.Module):
    ...
    def forward(self, input, return_image=True):
        args = self.args
        batch_size = input.size(0)
        in_chans = input.size(1)
        original_shape = input.shape
        # print("Batch_size: {}, In_chans: {}".format(x.size(0), x.size(1)))
        assert in_chans == args.in_chans
        out_chans = self.out_chans
        x = patchify_image(input, self.patch_size)  # [B,gh*gw*gd,ph*pw*pd*C]
        ...
        self.loss = self.criterion(input=all_x[:, -msk_length:, :], target=self.patch_norm(msk_x.detach()))
        ...
        if return_image:
            ...
            # recon = all_x.gather(dim=1, index=unshuffle_indices[:, :, None].expand(-1, -1, out_chans))

            # recon = recon * (x.var(dim=-1, unbiased=True, keepdim=True).sqrt() + 1e-6) + x.mean(dim=-1, keepdim=True)
            # recon: (8, 512, 4096) == (8, 128, 128, 128)
            recon = torch.cat([self.patch_norm(shuffled_x[:, :sel_length, :]), all_x[:, -msk_length:, :]], dim=1) \
            ...
            return self.loss, masked_x, recon
        else:
            return self.loss
