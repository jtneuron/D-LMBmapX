...
class SegTrainer(BaseTrainer):
    ...
    def build_model(self):
        ...
        if self.model_name != 'Unknown' and self.model is None:
            ...
            if args.pretrain is not None and args.pretrain_type == 'enc_dec' and os.path.exists(args.pretrain):
                ...
                # pdb.set_trace()
                if self.model_name in ['UNETR3D', 'SwinUNETR3D']:
    ...
    def val_seg(self, epoch, niters):
        ...
        for i, batch_data in enumerate(val_loader):
            ...
            with torch.cuda.amp.autocast(True):
                ...
                pre = torch.softmax(outputs, dim=1)[:, 1, ...]
                ...
                if epoch == 0 or (epoch + 1) % args.vis_freq == 0:
                    ...
                    tifffile.imwrite(
                        os.path.join(args.image_dir, 'val_output_' + str(i) + '_' + str(epoch + 1) + '.tiff'),
                        pre.squeeze().permute(2, 1, 0).detach().cpu().numpy())
    ...
    def vis_policy(self, niters=0, modality_idx=0, image_alpha=0.5):
        ...
        for b_idx, batch_data in enumerate(loader):
            ...
            # pdb.set_trace()
            output, policy_list = model(image,
                                        return_policy=True)  # policy list of [num_layers+1] 0-or-1 tensors with shape of [B, L+1]
