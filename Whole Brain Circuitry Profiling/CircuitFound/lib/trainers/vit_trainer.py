...
class VitTrainer(BaseTrainer):
    ...
    def build_optimizer(self):
        ...
        assigner = myutils.LayerDecayValueAssigner(list(args.layer_decay ** (num_layers + 1 - i) for i in range(num_layers + 2)))
    ...
    def evaluate(self, epoch, niters):
        ...
        meters = defaultdict(myutils.SmoothedValue)
        ...
        for i, (image, target) in enumerate(val_loader):
            ...
            if args.eval_metric == 'acc':
                ...
                batch_size = image.size(0)
                meters['loss'].update(value=loss.item(), n=batch_size)
                pred_list.append(myutils.concat_all_gather(output, args.distributed))
                targ_list.append(myutils.concat_all_gather(target, args.distributed))
        ...
        if args.eval_metric == 'auc':
            ...
            auc_list, mAUC = myutils.compute_aucs(pred_array, targ_array)
