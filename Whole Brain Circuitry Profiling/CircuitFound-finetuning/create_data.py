...
def load_tiff_convert_to_nifti(img_file, lab_file, img_out_base, anno_out, input_dim):
    img = imread(img_file)

    z = random.randint(0, img.shape[0] - input_dim)
    x = random.randint(0, img.shape[1] - input_dim)
    y = random.randint(0, img.shape[2] - input_dim)
    volume_chunk = img[z:z + input_dim, x:x + input_dim, y:y + input_dim].copy()

    img_itk = sitk.GetImageFromArray(volume_chunk.astype(np.float32))
    sitk.WriteImage(img_itk, join(img_out_base + "_0000.nii.gz"))

    if lab_file is not None:
        l = imread(lab_file)
        l[l > 0] = 1
        label_chunk = l[z:z + input_dim, x:x + input_dim, y:y + input_dim].copy()
        l_itk = sitk.GetImageFromArray(label_chunk.astype(np.float32))
...
def histogram_match_data(base, source, task_id, task_name, spacing, n_samples, input_dim,
    ...
    # skeletonized labels are used for training, while original labels are used for evaluation
    # labels_folder_path = join(data_path, "labels_sk") if flag else join(data_path, 'labels')
    labels_folder_path = join(data_path, "labels_sk")
    ...
    with tqdm(total=(len(volumes_path) + len(artifacts_path)) * 2, desc=f'original volume numbers') as pbar:
        ...
        if data_mix:
            for apath in artifacts_path:
                ...
                # artifact = gamma_correction(artifact, 1 / 4)
                # artifact = auto_contrast_3d(artifact, percent=0.5)
            ...
            print("aug_junk_num", aug_junk_num)
            if cutmix:
    ...
    data_len = (total_ori_volumes - aug_junk_num) * n_samples + aug_junk_num * (
        max(1, int(len(volumes_path) / len(artifacts_path) * n_samples)))
    # data_len = (total_ori_volumes - aug_junk_num) * n_samples
    print(data_len)
    ...
    with tqdm(total=data_len, desc='volumes numbers') as pbar:
        ...
        val_label_path = join(base, 'val', 'labels')
        vpaths = os.listdir(val_volume_path)
        for i, vpath in enumerate(vpaths):
            case = str(vpath).split(".")[0].split("-")[-1]
            volume = join(val_volume_path, "volume-" + case + ".tiff")
            label = join(val_label_path, "label-" + case + ".tif")
            casename = task_name + case
            img_out_base = join(imagests, casename)
            anno_out = join(labelsts, casename)
            res.append(
                p.starmap_async(load_tiff_convert_to_nifti, ((volume, label, img_out_base, anno_out, input_dim),)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--source', type=str, default=None, help='(if needed)data used for histogram match')
    parser.add_argument('--task_id', type=int, default=501, help='task id should be unique(better >200 to avoid conflict)')
    parser.add_argument('--data_mix', type=bool, default=True, help='artifact mix')
    parser.add_argument('--flag', type=bool, default=True, help='train')
    parser.add_argument('--task_name', type=str, default='Axon_validate_BS', help='task name')
    parser.add_argument('--match_flag', type=bool, default=True, help='histogram match')
    parser.add_argument('--cutmix', type=bool, default=True, help='cutmix')
    parser.add_argument('--join_flag', type=bool, default=False, help='join(matched and origin)')
    ...
    print(args)
    histogram_match_data(args.base, args.source, args.task_id, args.task_name, spacing,
                         3,  # number of samples(usually 2-6, depend on original train num)
                         128,  # input dimension(128*128*128)
                         args.data_mix, args.flag,  # artifact mix, train
                         # histogram match, cutmix, join(matched and origin)
                         args.match_flag, args.cutmix, args.join_flag)
