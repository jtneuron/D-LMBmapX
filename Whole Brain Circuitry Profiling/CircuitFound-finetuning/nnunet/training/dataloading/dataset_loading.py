...
class CustomDataset(Dataset):
    def __init__(self, folder, num_cases_properties_loading_threshold=1000):
        self.folder = folder
        self.num_cases_properties_loading_threshold = num_cases_properties_loading_threshold
        self.case_identifiers = self.get_case_identifiers(folder)
        self.case_identifiers.sort()
        self.dataset = {}

        for c in self.case_identifiers:
            self.dataset[c] = {
                'data_file': os.path.join(folder, f"{c}.npz"),
                'properties_file': os.path.join(folder, f"{c}.pkl"),
                'seg_from_prev_stage_file': os.path.join(folder,
                                                         f"{c}_segs.npz") if 'seg_from_prev_stage_file' else None
            }

        if len(self.case_identifiers) <= self.num_cases_properties_loading_threshold:
            self.load_all_properties()

    def get_case_identifiers(self, folder):
        # Implement your logic to retrieve case identifiers
        return []

    def load_all_properties(self):
        for case_id in self.dataset.keys():
            self.dataset[case_id]['properties'] = self.load_pickle(self.dataset[case_id]['properties_file'])

    def load_pickle(self, file):
        # Implement your pickle loading logic
        return {}

    def __len__(self):
        return len(self.case_identifiers)

    def __getitem__(self, idx):
        case_id = self.case_identifiers[idx]
        data = np.load(self.dataset[case_id]['data_file'])
        properties = self.dataset[case_id].get('properties', self.load_pickle(self.dataset[case_id]['properties_file']))
