...
def main():
    ...
    for batch, cond, paths in data:
        ...
        target_cond = get_cond(sample_target.cpu().numpy(), isTrain=False, typ=args.val_type_2)
        target_cond = {
            k: v.to(dist_util.dev())
        ...
        metrics.save_img_batch(batch, result_path, paths, "1_sources")
        metrics.save_img_batch(cond["cond_image"], result_path, paths, "2_cond_images")
        metrics.save_img_batch(sample_target, result_path, paths, "3_sample_target")
        metrics.save_img_batch(target_cond["cond_image"], result_path, paths, "4_target_cond")
        metrics.save_img_batch(sample_source, result_path, paths, "5_sample_source")
        if cond.get("target_image", None) is not None:
