...
sys.path.append(".")
import torch as th
from guided_diffusion.image_datasets_pair_sdc import load_val_data
from guided_diffusion import dist_util, logger, metrics
from guided_diffusion.script_util_sdc import (
...
def main():
    ...
    args.in_channels = args.in_channels // 3
    ...
    for batch, cond, paths in data:
        ...
        res_path = os.path.join(result_path, paths[0][0].split('/')[-2])
        os.makedirs(res_path, exist_ok=True)
        pre_path = paths[0][0].split('/')[-1][:-4]
        cur_path = paths[1][0].split('/')[-1][:-4]
        aft_path = paths[2][0].split('/')[-1][:-4]
        ...
        if args.continous:
            ...
            sample = sample[0]
            continous_img = rearrange(continous_img, 'b (c t) x y -> b c (t x) y', t=3)
        ...
        concat_imgs = rearrange(concat_imgs, 'b (c t) x y -> b c (t x) y', t=3)
        concat_imgs = metrics.tensor2img(concat_imgs)
        metrics.save_img(concat_imgs, '{}/{}_concatenate.png'.format(result_path, idx))

        cond_images = rearrange(cond["cond_image"], 'b (c t) x y -> b c (t x) y', t=3)
        cond_images = metrics.tensor2img(cond_images)
        metrics.save_img(cond_images, '{}/{}_cond_images.png'.format(result_path, idx))

        sources = rearrange(batch, 'b (c t) x y -> b c (t x) y', t=3)
        sources = metrics.tensor2img(sources)
        metrics.save_img(sources, '{}/{}_sources.png'.format(result_path, idx))

        translated = rearrange(sample, 'b (c t) x y -> t b c x y', t=3)
        translated_pre = metrics.tensor2img(translated[0])
        metrics.save_img(translated_pre, '{}/{}_translated_{}.png'.format(res_path, pre_path, idx))
        translated_cur = metrics.tensor2img(translated[1])
        metrics.save_img(translated_cur, '{}/{}_translated_{}.png'.format(res_path, cur_path, idx))
        translated_aft = metrics.tensor2img(translated[2])
        metrics.save_img(translated_aft, '{}/{}_translated_{}.png'.format(res_path, aft_path, idx))

        if cond.get("target_image", None) is not None:
            targets = rearrange(cond["target_image"], 'b (c t) x y -> b c (t x) y', t=3)
            targets = metrics.tensor2img(targets)
