...
from guided_diffusion.script_util import (
    ...
    create_model_segbranch_and_diffusion,
...
def main():
    ...
    model, diffusion = create_model_segbranch_and_diffusion(
    ...
    for batch, cond, paths in data:
        ...
        # translated = metrics.tensor2img_batch(sample)
        ...
        # metrics.save_img(translated, '{}/{}.png'.format(res_path, img_path))
        metrics.save_img_batch(sample, result_path, paths, "translated")

        if cond.get("original_image", None) is not None:
            # metrics.save_img(targets, '{}/{}_targets.png'.format(result_path, idx))
            _target_image = ((batch + 1) * 127.5)
            ...
            avg_psnr += peak_signal_noise_ratio(_target_image.cpu().numpy(), _sample.cpu().numpy(), data_range=255)
            # avg_ssim += ssim(_target_image.cpu().numpy(), _sample.cpu().numpy(), multichannel=False, data_range=255)
            avg_mse += mean_squared_error(_target_image.cpu().numpy(), _sample.cpu().numpy())
