...
class TrainLoop:
    ...
    def run_val(self):
        ...
        for batch, cond, _ in self.val_data:
            ...
            if cond.get("original_image", None) is not None:
                ...
                # avg_ssim += structural_similarity(_target_image.cpu().numpy(), _sample.cpu().numpy(), data_range=255)
        ...
        logger.log(f"{self.step} avg_mse:{avg_mse:.4e}")
        ...
        with open(val_log_path, 'a') as f:
            ...
            f.write(f"{self.step} avg_ssim:{avg_mse:.4e}\n")
    ...
    def save_best(self, ssim):
        def save_checkpoint(rate, params):
            state_dict = self.mp_trainer.master_params_to_state_dict(params)
            if dist.get_rank() == 0:
                logger.log(f"saving model {rate}...")
                filename = f"best_{rate}_{(self.step + self.resume_step):06d}_{ssim}.pt"
                with bf.BlobFile(bf.join(get_blob_logdir(), filename), "wb") as f:
                    th.save(state_dict, f)
                    self.model_saver.post_handle(bf.join(get_blob_logdir(), filename))
