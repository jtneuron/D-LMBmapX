    ...
    def epoch_train(self, epoch, niters):
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            for img in batch_images:
                ...
                img_min = img.min()
                img_max = img.max()
                normalized_img = (img - img_min) / (img_max - img_min + 1e-8)
    ...
    def val_mae(self, epoch, niters):
        ...
        for i, batch_data in enumerate(val_loader):
            ...
            if epoch == 0 or (epoch + 1) % args.vis_freq == 0:
                ...
                tifffile.imwrite(
                    os.path.join(args.image_dir, 'val_image' + str(i) + '_recon_' + str(epoch + 1) + '.tiff'),
                    recon.squeeze().permute(2, 1, 0).detach().cpu().numpy())
