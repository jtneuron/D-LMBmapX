...
def main():
    ...
    if len(args.resume_checkpoint) > 0:
        ...
        model_1.load_state_dict(state_dict['model_c1'])######################
        optimizer_1.load_state_dict(state_dict['optimizer_c1'])###########################
...
def run_test(args, model_1, model_2, val_dataloader, device, num_classes):
    ...
    with torch.no_grad():
        ...
        for img_dict in tqdm(val_dataloader):
            ...
            label1, label2 = img_dict['label1_i'], img_dict['label2']########
            label_test = img_dict["label1"]
                
            label_tmp = torch.zeros_like(label1, dtype=torch.uint8)
        
            # mask = label_test <= 5
            # label_tmp[mask] = label_test[mask]
            # mask = label1 > 5
            # label_tmp[mask] = label1[mask]
            ...
            for i in range(0, view_1_size, batch_size):
                ...
                warp_img1, warp_mask1, dvf_i = val_one_view(args, model_1, img1, img2, mask1, mask2, "val_view_1_")
            ...
            for i in range(0, view_2_size, batch_size):
                ...
                mask_test = extract_slices(label_test, i, batch_size, view_2_dim)
                B, C, H, W, D = img1.shape
                img1, img2, mask1, mask2 = permute_reg_imgs(img1, img2, mask1, mask2, B, C, H, W, D, view_2_dim)
                mask_test = permute_img(mask_test, B, C, H, W, D, view_2_dim)
                # TEST 只输入主脑区
                #mask1_with_main_label = mask1.clone()
                #mask1_with_main_label[mask1_with_main_label > 5] = 1
                #mask2_with_main_label = mask2.clone()
                ...
                warp_img1, warp_mask1, dvf_i = val_one_view(args, model_2, img1, img2, mask1, mask2, "val_view_2_")
            ...
            # view_1_warp_label1_nearest = view_1_warp_label1_nearest.cpu().numpy()
            # label2 = label2.cpu().numpy()
            # view_1_warp_label1_nearest[np.isin(view_1_warp_label1_nearest, [12, 13, 14, 15])] = 1
            # view_1_warp_label1_nearest[np.isin(view_1_warp_label1_nearest, [16, 17])] = 2
            # view_1_warp_label1_nearest[np.isin(view_1_warp_label1_nearest, [18, 19])] = 4
            # view_1_warp_label1_nearest[np.isin(view_1_warp_label1_nearest, [20])] = 5
            
            # label2[np.isin(label2, [12, 13, 14, 15])] = 1
            # label2[np.isin(label2, [16, 17])] = 2
            # label2[np.isin(label2, [18, 19])] = 4
            # label2[np.isin(label2, [20])] = 5
            
            view_1_warp_label1_nearest = torch.tensor(view_1_warp_label1_nearest, device=device)
            ...
            for i in range(num_classes):
                res[id1][i] = view_1_metric_dict[f'dice_class_{i}']

            save_dir = os.path.join(args.logdir, "warp_data", id1)
            os.makedirs(save_dir, exist_ok=True)
            sitk.WriteImage(sitk.GetImageFromArray(view_1_warp_volume1.cpu().numpy()), os.path.join(save_dir, "warp_volume.nii.gz"))
            sitk.WriteImage(sitk.GetImageFromArray(volume2.cpu().numpy()), os.path.join(save_dir, "fixed.nii.gz"))
            sitk.WriteImage(sitk.GetImageFromArray(view_1_warp_label1_nearest.cpu().numpy()), os.path.join(save_dir, "warp_label.nii.gz"))
...
def create_argparser():
    defaults = dict(
        view_1_dim=2,
        view_2_dim=4,
        data_dir=r"/phz/cross_stage/output_fetal/33_25_to_29/train_mutual/data.toml",
        lr=1e-4,
        batch_size=16,
        log_interval=10,
        save_interval=50,
        resume_checkpoint="/phz/cross_stage/output_fetal/33_25_to_29/train_mutual/state000100.pt",
        logdir="/phz/cross_stage/output_fetal/33_25_to_29/train_mutual/test2",
        gpu='0',
        max_save_num=5,
        val_interval=5,
        epoches=200,
