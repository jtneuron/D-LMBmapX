        ...
        for level, convs in enumerate(self.decoder):
            ...
            if not self.half_res or level < (self.nb_levels - 2):
                ...
                if x.shape[2:] != x_history[-1].shape[2:]:
                    # x = F.interpolate(x, size=x_history[-1].shape, mode='trilinear', align_corners=True)
                    x = F.interpolate(x, size=x_history[-1].shape[2:], mode='bilinear' if x.ndim==4 else 'trilinear', align_corners=True)
...
class VxmDense(nn.Module):
    ...
    def __init__(self,
        ...
        self.transformer = layers.SpatialTransformer()
