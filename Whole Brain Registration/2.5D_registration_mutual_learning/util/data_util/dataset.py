import sys
sys.path.append('/syq/registration_3')
...
class TrainPairDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img['label1_u'] = read_3d_mask(img1_path, 'u')
        img['label1_i'] = read_3d_mask(img1_path, 'i')
    ...
    def as_type_to_tensor(img):
        for key, value in img.items():
            if isinstance(value, np.ndarray):
                # TEST
                if key == 'label1' or key == 'label1_u' or key == 'label1_i' or key == 'label2':
...
if __name__ == '__main__':
    import json
    with open('/mnt/18TB_HDD2/suyuqin/registration_3/datasets/test.json', 'r') as f:
    ...
    print(train_dataset[n]['volume1'].shape, train_dataset[n]['label1_u'].shape, train_dataset[n]['id1'])