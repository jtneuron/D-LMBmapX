...
def main():
    ...
    model_1 = create_model(**args_to_dict(args, voxelmorph_defaults().keys()))
    model_2 = create_model(**args_to_dict(args, voxelmorph_defaults().keys()))
    ...
    if args.data_dir.endswith('.toml'):
        ...
        raise ValueError("data_dir should be a toml or json file")
    ...
    run_test(args, model_1, model_2, test_dataloader, device, num_classes)
...
def run_test(args, model_1, model_2, test_dataloader, device, num_classes):
    model_1.eval()
    model_2.eval()
    batch_size = args.batch_size
    view_1_dim = args.view_1_dim
    view_2_dim = args.view_2_dim
    imgshape = (1, 1, 800, 1140, 1320)
    ave_view1 = np.zeros(imgshape, 'float')
    ave_view2 = np.zeros(imgshape, 'float')

    # 分脑区进行处理，设置阈值0.5， 将所有label累加后除以脑子数得到平均label 此时label会出现数据分布不均匀，凡是小于label值 * 阈值的体素点都置为0，否则置为1
    ave_view1_multi_region = {str(i): np.zeros(imgshape, 'float') for i in range(1, num_classes + 1)}
    ave_view2_multi_region = {str(i): np.zeros(imgshape, 'float') for i in range(1, num_classes + 1)}
    ave_view1_label = np.zeros(imgshape, 'float')
    ave_view2_label = np.zeros(imgshape, 'float')
    counter = 0
    with torch.no_grad():
        for img_dict in tqdm(test_dataloader):
            ...
            for i in range(0, view_1_size, batch_size):
                ...
                # mask1 = mask1.to(device) if mask1 != [] else mask1
                # mask2 = mask2.to(device) if mask2 != [] else mask2
            ...
            ave_view1 += view_1_warp_volume1.squeeze().cpu().numpy()  #
            for i in range(1, num_classes+1):
                ave_view1_multi_region[str(i)] += (view_1_warp_label1_nearest.squeeze().cpu().numpy() == i)*i
            # ave_view1_label += view_1_warp_label1_nearest.squeeze().cpu().numpy()
            ...
            ave_view2 += view_2_warp_volume1.squeeze().cpu().numpy()  #
            for i in range(1, num_classes+1):
                ave_view2_multi_region[str(i)] += (view_2_warp_label1_nearest.squeeze().cpu().numpy() == i)*i
            # ave_view2_label += view_2_warp_label1_nearest.squeeze().cpu().numpy()
        ...
        if args.save_img:
            ...
            threshold = 0.45
            for i in range(1, num_classes+1):
                value1 = ave_view1_multi_region[str(i)]
                value1 /= counter

                value1[value1 < threshold * i] = 0
                value1[value1 != 0] = np.max(value1)

                # 形态学后处理
                import skimage
                value1 = skimage.morphology.closing(value1)
                value1 = skimage.morphology.opening(value1)
                ave_view1_label[value1 != 0] = i

                value2 = ave_view2_multi_region[str(i)]
                value2 /= counter
                value2[value2 < threshold * i] = 0
                value2[value2 != 0] = np.max(value2)
                # 形态学后处理
                import skimage
                value2 = skimage.morphology.closing(value2)
                value2 = skimage.morphology.opening(value2)
                ave_view2_label[value2 != 0] = i
            # ave_view1_label /= counter
            # ave_view2_label /= counter
            base_dir = os.path.join(logger.get_dir(), 'ave')
            os.makedirs(base_dir, exist_ok=True)
            write_image_3d(ave_view1[0][0] * 255, os.path.join(base_dir, f"{'view1'}_ave_volume.nii.gz"), np.uint8)
            write_image_3d(ave_view2[0][0] * 255, os.path.join(base_dir, f"{'view2'}_ave_volume.nii.gz"), np.uint8)
...
def create_argparser():
    defaults = dict(
        view_1_dim=2,
        view_2_dim=4,
        data_dir="/media/user/a7ede1e7-8011-4a0c-903a-6a8652a8de87/workspace/phz/build_atlas/registration_3/datasets/data_config/P14_ave_10um.toml",
        checkpoint="/media/user/a7ede1e7-8011-4a0c-903a-6a8652a8de87/workspace/phz/build_atlas/registration_3/output/P14_ave_10um_test2/state000080.pt",
        batch_size=1,
        logdir="/media/user/a7ede1e7-8011-4a0c-903a-6a8652a8de87/workspace/phz/build_atlas/registration_3/output/P14_ave_10um_test2/test/test_shai",
        gpu='',
        registration_type=1,
        save_img=True
    )
